...
aws lambda create-function \
	...
	--code S3Bucket="${BUCKET_NAME}",S3Key="${ZIPFILE}" \
	--output json

ARN=$(aws lambda list-layer-versions --layer-name ${LAYER_NAME} --query 'LayerVersions[].LayerVersionArn' --region ${REGION})
echo "Lambda layer is $ARN"
